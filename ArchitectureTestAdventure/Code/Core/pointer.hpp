...
namespace ata {
    ...
    class pointer {
    public:
        constexpr pointer(uint64_t index, uint64_t generation) noexcept
        ...
        constexpr uint64_t index() const noexcept { return m_index; }
        constexpr uint64_t generation() const noexcept { return m_generation; }
    ...
    public:
        using handle_t = pointer<_Ty>;

        typed_pointer_storage()  = default;
        ~typed_pointer_storage() = default;

        handle_t create(_Ty value) {
            ...
            if (!m_freeList.empty()) {
                ...
                index = static_cast<uint64_t>(m_slots.size());

                m_slots.emplace_back(_MySlot{ std::move(value) });
            }
            return handle_t(index, m_slots[index].generation);
        }
        ...
        _Ty* get(handle_t handle) noexcept {
            std::shared_lock lock(m_mutex);
            if (!is_valid_locked(handle)) return nullptr;
            return std::addressof(m_slots[handle.m_index].object);
        }

        const _Ty* get(handle_t handle) const noexcept {
            std::shared_lock lock(m_mutex);
            if (!is_valid_locked(handle)) return nullptr;
            return std::addressof(m_slots[handle.m_index].object);
        }

        bool is_valid(handle_t handle) const noexcept {
            std::shared_lock lock(m_mutex);
    ...
    private:
        bool is_valid_locked(handle_t handle) const noexcept {
        ...
        struct _MySlot {
            ...
            _MySlot(_Ty&& value, uint64_t generation = 0, bool alive = true)
                : object(std::move(value)), generation(generation), alive(alive) {}

            _MySlot(const _Ty& value, uint64_t generation = 0, bool alive = true)
